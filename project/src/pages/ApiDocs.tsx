...
const endpoints: Endpoint[] = [
  ...
  // Offer endpoints
  { method: 'POST', path: '/offer/createOffr', title: 'Create Offer', description: 'Create a new offer in the platform.', auth: true, category: 'Offers', params: ['title', 'advertiserId', 'privacyLavel', 'description', 'eventType', 'eventName', 'revenue', 'payout', 'trackingUrl', 'impressionUrl'], sample: `curl -X POST https://cl.repowire.com/offer/createOffr -H "Authorization: Bearer <token>" -F "title=API Test Offer" -F "advertiserId=1" -F "eventType=CPI" -F "eventName=install" -F "revenue=1.5" -F "payout=1.0"` },
  { method: 'GET', path: '/offer/offerList', title: 'Offer List', description: 'Fetch paginated offers with optional search.', auth: true, category: 'Offers', params: ['page', 'search'], sample: `curl -H "Authorization: Bearer <token>" "https://cl.repowire.com/offer/offerList?page=1&search="` },
